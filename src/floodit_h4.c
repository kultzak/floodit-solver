...
void recombination(tmapa mo, tplano pop[], int k, float lambda, int ncores)
{
	...
	for (int son = 10; son < 20; ++son) //local para salvar os filhos recombinados
	{
		...
		// printf("FILHO: %d\n", pop[son].passos); //imprime indivíduo gerado
		// for (int i = 0; i < pop[son].passos; i++)
		// 	printf("%d ", pop[son].cor[i]);
		// printf("\n");
	}
	printf("RESULTADOS: \n");
	int c = 0;
...
//-----------------------//ORDENAR POPULAÇÃO//-----------------------//
	...
	while (start < endwait) //função timer para englobar
	{
		...
		printf("MELHOR RESULTADO PARCIAL: %d\n", ini_pop[0].passos);
...
//-----------------------//APRESENTA O MELHOR RESULTADO//-----------------------//
	printf("MELHOR RESULTADO FINAL: %d\n", ini_pop[0].passos);
